  ...
  portfolioCategories: [
    {
      ...
      items: [
        { id: "an-1", label: "Sample Project 1", thumb: "assets/images/portfolio/Zomato.png", zipUrl: "assets/Healthy Restaurent.zip", entryFile: "index.html"},
        { id: "an-2", label: "Sample Project 2", thumb: "assets/images/portfolio/Web Banner.png", zipUrl: "animate/Web Banner 1200 x 400.zip" },
...
async function openZipProject(item) {
  ...
  try {
    ...
    const indexEntry = item.entryFile
      ? fileEntries.find(f => f.name.toLowerCase() === item.entryFile.toLowerCase()
          || f.name.toLowerCase().endsWith("/" + item.entryFile.toLowerCase()))
      : fileEntries.find(f => f.name.toLowerCase().endsWith("index.html"));
    if (!indexEntry) throw new Error("No index.html found in this .zip");

    // Resolve a path referenced from inside some file at `fromDir` (a
    // zip-relative directory, e.g. "" for root or "sub/") against the
    // urlMap of extracted files. Adobe Animate exports append a
    // cache-busting "?<timestamp>" query string to every asset reference
    // (both in <script src="script.js?169...">  tags AND inside script.js's
    // own CreateJS manifest, e.g. src:"images/foo.png?169..."), so the
    // query string has to be stripped before matching, and matching has to
    // happen against BOTH html and js/css file contents, not just html tag
    // attributes.
    function resolveInZip(fromDir, rawPath) {
      if (/^(https?:)?\/\//.test(rawPath) || rawPath.startsWith("data:")) return null;
      const noQuery = rawPath.split(/[?#]/)[0];
      const stack = [];
      for (const part of (fromDir + noQuery).split("/")) {
      ...
      const foundKey = Object.keys(urlMap).find(k => k.toLowerCase() === lowerResolved);
      return foundKey ? urlMap[foundKey] : null;
    }

    // Generic rewrite: swap any quoted asset path — whether it's an HTML
    // attribute or a JS string literal inside a CreateJS manifest — for its
    // extracted blob: URL. This is what a plain src/href regex on the HTML
    // alone misses, because the image manifest path lives inside script.js,
    // not index.html.
    function rewriteAssetPaths(text, fileDir) {
      // Path segment allows spaces (Animate exports frequently ship
      // filenames like "945 x 216_atlas_1.png") — it can only exclude the
      // quote chars and angle brackets, and the closing quote is matched
      // via a backreference to whichever quote (' or ") opened the string.
      return text.replace(
        /(["'])([^"'<>]+?\.(?:png|jpe?g|gif|svg|webp|json|css|js|html?|mp3|mp4|ogg|woff2?|ttf|otf))(\?[^"'<>]*)?\1/gi,
        (match, quote, path) => {
          const url = resolveInZip(fileDir, path);
          return url ? `${quote}${url}${quote}` : match;
        }
      );
    }

    // Pass 1: rewrite internal asset references inside every JS/CSS file
    // first, and replace their blob: URLs in urlMap with the rewritten
    // version, so the HTML pass below picks up JS that already points at
    // real image/font/etc. blobs instead of the zip's original relative paths.
    for (const entry of fileEntries) {
      const lower = entry.name.toLowerCase();
      if (lower.endsWith(".js") || lower.endsWith(".css")) {
        const dir = entry.name.includes("/") ? entry.name.slice(0, entry.name.lastIndexOf("/") + 1) : "";
        const raw = await entry.async("string");
        const rewritten = rewriteAssetPaths(raw, dir);
        URL.revokeObjectURL(urlMap[entry.name]);
        urlMap[entry.name] = URL.createObjectURL(new Blob([rewritten], {
          type: lower.endsWith(".js") ? "application/javascript" : "text/css"
        }));
      }
    }

    // Pass 2: rewrite index.html's own references (script/link/img tags),
    // now pointing at the already-fixed JS/CSS blobs from pass 1.
    ...
    html = rewriteAssetPaths(html, indexDir);
